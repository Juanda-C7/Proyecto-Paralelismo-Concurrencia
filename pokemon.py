...
def download_pokemon_concurrent(n=150, dir_name='pokemon_dataset', max_workers=32):
...
def process_pokemon_parallel(dir_origin='pokemon_dataset', dir_name='pokemon_processed', max_workers=8): #Utilizar maximo 8 cores
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(process_single_image, img, dir_origin, dir_name): img for img in images}
...
if __name__ == '__main__':
    print('=' * 60)
    print_pikachu()
    print('   POKEMON IMAGE PROCESSING PIPELINE (Optimized)')
    print('=' * 60)
